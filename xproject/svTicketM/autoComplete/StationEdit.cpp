// StationEdit.cpp: Implementierungsdatei
//
...
BEGIN_MESSAGE_MAP(CStationEdit, CWnd)
	//{{AFX_MSG_MAP(CStationEdit)
...
void CStationEdit::SetMode(int iMode)
{
	if(m_iType == -1)
		Init();
...
void CStationEdit::AddSearchStrings(std::vector<station_name>& _vec)
{
	int i = 0;
	//LPCTSTR str;
	if(m_iType == -1)
	...
	while(str);
#else
// 	for (int idx = 0; idx < stat_name_vec.size(); ++idx)
// 	{
// 		m_Liste.AddSearchString((stat_name_vec[idx]._stat_cn).c_str());
// 	}
	m_Liste.AddSearchLists(_vec);
#endif
...
// void CStationEdit::AddSearchString(LPCTSTR lpszString)
// {
// 	if(m_iType == -1)
// 	{
// 		ASSERT(0);
...
void CStationEdit::RemoveSearchAll()
{
	if(m_iType == -1)
	{
		ASSERT(0);
...
void CStationEdit::OnKillfocus()
{
	if (_select_only)
	{
		GetWindowText(m_EditText);
		int idx = m_Liste.FindStringExact(-1, m_EditText);
		if (m_Liste.FindStringExact(-1, m_EditText) == -1)
		{
			SetWindowText(_T(""));
		}
	}
...
void CStationEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
bool CStationEdit::HandleKey(UINT nChar, bool m_bFromChild)
{
...
int CStationEdit::FindSepLeftPos(int pos,bool m_bIncludePrefix)
{
...
int CStationEdit::FindSepLeftPos2(int pos)
{
	int len = m_EditText.GetLength();
	TCHAR ch;
...
int CStationEdit::FindSepRightPos(int pos)
{
    int len = m_EditText.GetLength();
    TCHAR ch;
    int i = 0;
...
LONG CStationEdit::OnUpdateFromList(UINT lParam, LONG /*wParam*/)
{
	UpdateData(true);
...
BOOL CStationEdit::PreTranslateMessage(MSG* pMsg)
{
...
// int CStationEdit::AddString( LPCTSTR lpszString )
// {
// 	if(m_iType == _COMBOBOX_)
// 	{
...
int CStationEdit::SetDroppedWidth( UINT nWidth )
{
	if(m_iType == _COMBOBOX_)
	{
...
int CStationEdit::FindString( int nStartAfter, LPCTSTR lpszString )
{
	if(m_iType == _COMBOBOX_)
	{
...
int CStationEdit::SelectString( int nStartAfter, LPCTSTR lpszString )
{
	if(m_iType == _COMBOBOX_)
	{
...
void CStationEdit::ShowDropDown(BOOL bShowIt)
{
	if(m_iType == _COMBOBOX_)
	{
...
void CStationEdit::ResetContent()
{
	if(m_iType == _COMBOBOX_)
	{
...
int CStationEdit::GetLBText( int nIndex, LPTSTR lpszText )
{
	if(m_iType == _COMBOBOX_)
	{
...
void CStationEdit::GetLBText( int nIndex, CString& rString )
{
	if(m_iType == _COMBOBOX_)
	{
