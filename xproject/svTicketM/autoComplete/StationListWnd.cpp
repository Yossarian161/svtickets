...
CStationListWnd::CStationListWnd()
{
	m_lTopIndex = 0;
	m_lCount = 0;
	m_ItemHeight = 16;
...
void CStationListWnd::OnActivateApp(BOOL bActive, DWORD dwThreadID)
{
#if (_MSC_VER >= 1300)
...
BEGIN_MESSAGE_MAP(CStationListWnd, CWnd)
	//{{AFX_MSG_MAP(CStationListWnd)
	ON_WM_PAINT()
	ON_WM_SIZE()
	ON_WM_ERASEBKGND()
	ON_WM_NCPAINT()
...
/////////////////////////////////////////////////////////////////////////////
// Behandlungsroutinen f黵 Nachrichten CStationListWnd

void CStationListWnd::DrawItem(CDC* pDC,long m_lItem,long width)
{
...
void CStationListWnd::Init(CWnd *pWnd)
{
	VERIFY(m_VertBar.Create(WS_VISIBLE|SBS_VERT|SBS_LEFTALIGN,
	                        CRect(0,0,GetSystemMetrics(SM_CYVSCROLL),0),this,0));
...
void CStationListWnd::SetScroller()
{
	CRect rcWnd,rcBar;
	GetClientRect(rcWnd);
	...
	if(m_VertBar.GetSafeHwnd())
	{
		...
		//rcBar.bottom -= GetSystemMetrics(SM_CYHSCROLL);	// 取消竖直滚动条下边空白区 2015-01-19 @jianyaoy@gmail.com
...
void CStationListWnd::OnSize(UINT nType, int cx, int cy)
{
...
long CStationListWnd::ScrollBarWidth()
{
	if(m_VertBar.IsWindowVisible())
...
BOOL CStationListWnd::OnEraseBkgnd(CDC* /*pDC*/)
{
	return false;
}
...
void CStationListWnd::OnNcPaint()
{
	CWindowDC dc(this);
...
void CStationListWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CStationListWnd::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS FAR* lpncsp)
{
...
int CStationListWnd::HitTest(CPoint point)
{
	CRect rcItem;
	CRect rcWnd;
...
#if _MSC_VER > 1400
afx_msg LRESULT CStationListWnd::OnNcHitTest(CPoint point)
#else
afx_msg UINT CStationListWnd::OnNcHitTest(CPoint point)
#endif
{
	CRect rectClient;
...
void CStationListWnd::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CStationListWnd::OnRButtonDown(UINT nFlags, CPoint point)
{
...
BOOL CStationListWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	CRect rectClient;
	CPoint ptCursor;
	...
	if(rectClient.PtInRect(ptCursor)) // Vergr鲞erungs-Cursor
	{
...
void CStationListWnd::InvalidateAndScroll()
{
	m_VertBar.SetScrollPos(m_lTopIndex,true);
	Invalidate();
	DoPaintMessageLoop();
...
bool CStationListWnd::EnsureVisible(int item, bool m_bWait)
{
...
bool CStationListWnd::SelectItem(int item)
{
	if(item > m_lCount)
		return false;
...
int CStationListWnd::FindStringExact( int nStartAfter, LPCTSTR lpszString )
{
...
* NEW: m_bDisplayOnly
*/
int CStationListWnd::FindString(int nStartAfter, LPCTSTR lpszString, bool m_bDisplayOnly)
{
	...
	if(!m_bDisplayOnly)
	{
		...
		CString m_str_tmp1, m_str_tmp2, m_str_tmp3;
		...
		for (unsigned int idx = nStartAfter+1; idx < stat_name_list.size(); ++idx)
		{
			if(m_PrefixChar)
				m_Str1 = m_PrefixChar;
			else
...
#if _UNICODE
			m_str_tmp1 = m_Str1 + win32_utils::AnsiToUnicode(stat_name_list[idx].stat_spell_logo.c_str());
			m_str_tmp2 = m_Str1 + win32_utils::AnsiToUnicode(stat_name_list[idx].stat_spell_full.c_str());
			m_str_tmp3 = m_Str1 + win32_utils::AnsiToUnicode(stat_name_list[idx].stat_cn.c_str());
#else
			m_str_tmp1 = m_Str1 + stat_name_list[idx].stat_spell_logo.c_str();
			m_str_tmp2 = m_Str1 + stat_name_list[idx].stat_spell_full.c_str();
			m_str_tmp3 = m_Str1 + stat_name_list[idx].stat_cn.c_str();
#endif

			m_str_tmp1.MakeUpper();
			m_str_tmp2.MakeUpper();
			m_str_tmp3.MakeUpper();

			if(m_lMode & _MODE_FIND_ALL_)
			{
				if(m_str_tmp1.Find(m_Str2) >= 0 || m_str_tmp2.Find(m_Str2) >= 0 || m_str_tmp3.Find(m_Str2) >= 0)
				{
#if _UNICODE
					m_DisplayList.Add(win32_utils::AnsiToUnicode(stat_name_list[idx].stat_cn.c_str()));
#else
					m_DisplayList.Add(stat_name_list[idx].stat_cn.c_str());
#endif
				}
			}
	...
	if(m_lCount)
	{
		...
		// comment: 车站名列表按12306提供的排序，此处不重新排序 [1/19/2015 Administrator]
...
int CStationListWnd::SelectString(LPCTSTR lpszString )
{
...
bool CStationListWnd::GetText(int item, CString& m_Text)
{
...
void CStationListWnd::OnShowWindow(BOOL bShow, UINT nStatus)
{
	if(bShow)
	{
...
void CStationListWnd::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
...
CString CStationListWnd::GetString()
{
...
void CStationListWnd::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
CString CStationListWnd::GetNextString(int nChar)
{
	switch(nChar)
	{
	case VK_DOWN:
...
void CStationListWnd::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CStationListWnd::OnTimer(UINT nIDEvent)
{
...
void CStationListWnd::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	if(GetSafeHwnd())
	{
		// Vers. 1.2
...
int CStationListWnd::CompareString(const void* p1, const void* p2)
{
...
void CStationListWnd::SortList(CStringArray& m_List)
{
