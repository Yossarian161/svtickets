...
{
	m_xpoint = CPoint(nRow, nCol);

	// If url is present then change text color
	if (HasUrl(GetText()))
		SetTextClr(m_clrUrl);

	// 给字体添加下划线 [@Byxing 2015-1-20 22:34]
	LOGFONT* pfont = GetFont();
	pfont->lfUnderline = 1;
	SetFont(pfont);
...
{
#if 0	// modify [@Byxing 2015-1-21 9:46]
...
#else	// 给gridctrl的父窗口发送一个click自定义消息

	CGridCtrl* grid = GetGrid();
	CString strURL;
	if (GetAutoLaunchUrl() && OverURL(PointCellRelative, strURL))
		//GetParent(grid->GetSafeHwnd())->SendMessage(WM_GRID_CELL_CLICK, (void *)m_xpoint, NULL);
		SendMessage(GetParent(grid->GetSafeHwnd()), WM_GRID_CELL_CLICK, WPARAM(m_xpoint.x), LPARAM(m_xpoint.y));
#endif
}
...
BOOL CGridURLCell::OnSetCursor()
{
    ...
    {
#if _MSC_VER < 1400		//modify @jianyaoy
...
BOOL CGridURLCell::HasUrl(CString str)
{
    ...
    for (int i = 0; i < nNumPrefixes; i++)
        ...
        if (str.Find(g_szURIprefixes[i].szURLPrefix) >= 0)
            return TRUE;
	// change return true [@Byxing 2015-1-20 22:16]
	return TRUE;
//    return FALSE;
}
